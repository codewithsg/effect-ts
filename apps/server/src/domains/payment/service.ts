...
const mockPaymentGateway = (paymentId: number, userId: number) => Effect.gen(function* () {
    yield* Effect.annotateCurrentSpan("gateway.status", "started");
    yield* Effect.sleep("100 millis");
    yield* Effect.annotateCurrentSpan("gateway.status", "success");
    return true;
}).pipe(
    Effect.withSpan("PaymentService.mockPaymentGateway", { attributes: { paymentId, userId } }),
...
const makeCheckout = (
    ...
    (Effect.gen(function* () {
        ...
        for (const item of input.products) {
            ...
            if (product.status === PRODUCT_STATUS.INACTIVE) {
                yield* Effect.logWarning("Checkout blocked — inactive product").pipe(
                    Effect.annotateLogs({
                        event: "checkout",
                        outcome: "error",
            ...
            if (product.stock < item.quantity) {
                yield* Effect.logWarning("Checkout blocked — insufficient stock").pipe(
                    Effect.annotateLogs({
                        event: "checkout",
                        outcome: "error",
        ...
        if (user.availableAmount < totalAmount) {
            ...
            yield* Effect.logWarning("Checkout blocked — insufficient balance").pipe(
                Effect.annotateLogs({
                    event: "checkout",
                    outcome: "error",
    ...
    // Cast here — before .pipe() — so tapErrorTag can see the full CheckoutError union.
            ...
            Effect.withSpan("PaymentService.checkout", { attributes: { userId: input.userId, cartSize: input.products.length } }),
            Effect.annotateLogs({ "user.id": input.userId, "cart.item_count": input.products.length }),
            ...
            Effect.tapErrorTag("UserNotFoundError", (err) =>
                Effect.logError("Checkout failed — user not found").pipe(
                    Effect.annotateLogs({
            ...
            Effect.tapErrorTag("UserDecodingError", (err) =>
                Effect.logError("Checkout failed — user record could not be decoded").pipe(
                    Effect.annotateLogs({
                        event: "checkout", outcome: "error",
            ...
            Effect.tapErrorTag("ProductNotFoundError", (err) =>
                Effect.logError("Checkout failed — product not found").pipe(
                    Effect.annotateLogs({
                        event: "checkout", outcome: "error",
            ...
            Effect.tapErrorTag("ProductDecodingError", (err) =>
                Effect.logError("Checkout failed — product record could not be decoded").pipe(
                    Effect.annotateLogs({
                        event: "checkout", outcome: "error",
            ...
            Effect.tapErrorTag("DatabaseError", (err) =>
                Effect.logError("Checkout failed — database error").pipe(
                    Effect.annotateLogs({
                        event: "checkout", outcome: "error",
            ...
            Effect.catchDefect((defect) => Effect.gen(function* () {
                yield* Effect.logFatal("Unexpected defect in checkout — programming error or runtime fault").pipe(
                    Effect.annotateLogs({
                        event: "checkout",
                        outcome: "defect",
                        "error.type": "Defect",
